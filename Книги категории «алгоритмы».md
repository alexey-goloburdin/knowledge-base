...
cssclasses:
  - wide-85
---
# Алгоритмы (`$= dv.pages("").where(page => {if (page["Тип"] && page["Тип"].path == "types/Книга.md" && !["templates", "templater"].includes(page.file.folder) && page.Категории.contains("алгоритмы")) {return page;}}).length`)
...
const booksFiles = window.books.getBooks()
    .where(p => p["Категории"].contains("алгоритмы"))
