...
for (const book of booksFiles) {
	const categories = (book.Категории ? book.Категории : []).map(
	    c => `<span class="category" style="background-color: ${b.getCategoryColor(c)}">${c}</span>`
	).join(" ");
    booksHtml.push(`<div class="book">
        ...
        ${book.Progress}
        начал ${b.formatDate(book["Начал читать"])}<br>
        <div class="categories">${categories}</div>
        <div class="pages">${book.Страниц} стр.</div>
    </div>`);
}

dv.el("div", booksHtml.join(""), {cls: "books"});
```
