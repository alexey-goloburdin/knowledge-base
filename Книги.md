...
dv.el("div", window.books.renderBooks(booksFiles, "начал ${formatDate(book['Начал читать'])}<br>"), {cls: "books"});
*/
```
...
dv.el("div", window.books.renderBooks(booksFiles, "добавлено ${formatDate(book['Добавлена в базу'])}<br>"), {cls: "books"});
```
# Прочитанное в 2025
