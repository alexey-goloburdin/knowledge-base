...
cssclasses:
  - wide-85
---
# Rust (`$= dv.pages("").where(page => {if (page["Тип"] && page["Тип"].path == "types/Книга.md" && !["templates", "templater"].includes(page.file.folder) && page.Категории.contains("rust")) {return page;}}).length`)
...
const booksFiles = window.books.getBooks()
    .where(p => p["Категории"].contains("rust"))
