...
Ещё датаклассы окей для DTO, Data Transfer Object, то есть структур данных, которые гоняются между слоями приложения. Передаём данные между слоями приложения не кортежами с кучей элементов, не словарями, не типизированными словарями, а именно датаклассами. Причём надо стараться почаще использовать в таких случаях для датаклассов тоже параметры `frozen=True` и `slots=True`, потому что для DTO это естественно. Ну и конечно у DTO нет никакой бизнес-логики, это просто контейнер для данных с прописанной структурой и типизацией.
