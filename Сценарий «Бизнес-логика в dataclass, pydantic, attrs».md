...
Обратите внимание — нам тут пришлось самим делать метод `__init__`, самим делать метод `__repr__`, мы не получили понятного списка полей экземпляра класса, они задаются в `__init__`, но это не очень наглядно.
