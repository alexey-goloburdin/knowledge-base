...
Я хочу сказать, что в большинстве случаев лучший выбор для написания БЛ это простые обычные классы, не датаклассы, не pydantic и не attrs-классы. Сейчас поговорим, почему так, это большой и важный вопрос об архитектуре кода и правильном использовании объектно-ориентированного программирования.
...
И, прежде чем посмотреть, как он всё будет в датаклассах, небольшое отступление — для меня удивительно, но я иногда кого-то в офлайне встречаю, на конференциях где-то, и мне часто говорят, что не знают о том, что у меня есть курс веб-разработке. Я пишу всегда об этом где-то под видео, но вероятно надо говорить и в самом видео. Друзья, я уже несколько лет разрабатываю и веду свой огромный авторский курс «Хардкорная веб-разработка», на который зову всех, кто хочет прокачать свои навыки. Там всё, начиная от азов Computer Science, командной строки Linux, языков программирования Python, JS и TS, изучаем плотно PostgreSQL, изучаем бэкенд, фронтенд, вёрстку, тестирование ПО, вопросы архитектуры, качества и процесса разработки ПО и много всего смежного. Приходите.

Стоит дорого, потому что я вкладываю непомерное количество своего времени, своего труда и своей души в этот курс, и цена будет ещё дороже с выходом новых глав. Там огромное количество материала, заданий, созвонов и тд. Заданий вот уже сейчас почти тысяча, всё работает на нашей специально разработанной платформе. Надо кстати снять отдельный материал о платформе и том, как мы её пишем, как-нибудь сделаю. Мы её делаем уже три года, там сейчас 280 тыс строк кода без учёта комментов и пустых строк, есть что рассказать-показать.

Ссылку на курс видите на экране и она есть в описании. Можно оплатить в рассрочку, может оплатить ваш работодатель. Курс проходят как начинающие, так и сотрудники без преувеличения всех компаний российского бигтеха — Яндекс, VK, Avito, Сбер, Т-Банк, Лаборатория Касперского и тд. Приходите, если готовы работать! Без гарантий трудоустройства и без прочей ерунды, только мясо и хардкор.
...
Ещё это плохо, потому что внутреннее состояние класса публично видно клиенту класса и клиент класса видит в том числе детали реализации класса, что плохо. Во внутренних полях класс может хранить данные, которые относятся к деталям реализации, и открывать их публично клиентам класса не нужно. dataclass же нас к этому подталкивает. По сути dataclass поощряет анемичную модель, которая содержи только данные, а функции по их обработке находятся где-то в другом месте, и dataclass делает _правильную_ инкапсуляцию **дороже и сложнее**,  чем в обычном классе. Сложнее нормально скрыть детали реализации, нормально защитить инварианты и тд и тд и тд.
...
Во-вторых, как по мне основная задача Pydantic — это удобная работа с JSON, например, валидация входного JSON в вашем веб-сервисе. Для этого Pydantic отлично подходит и здесь он уместен в том же фреймворке FastAPI или где угодно. Также и в обратную сторону, собрать JSON из вложенных сущностей удобно с Pydantic. Но использовать Pydantic где-то в  доменном слое я считаю неуместным.
