...
Ещё это плохо, потому что внутреннее состояние класса публично видно клиенту класса и клиент класса видит в том числе детали реализации класса, что плохо. Во внутренних полях клиент может хранить данные, которые относятся к своим деталям реализации, и открывать их публично клиентам класса не нужно. dataclass же нас к этому подталкивает. Таким образом, dataclass не даёт нам нормально инкапсулировать логику в класс, нормально скрыть детали реализации, нормально защитить инварианты и тд и тд и тд.

Можно ли как-то с этим бороться в случае использования датакласса? Можно, назвав поля экземпляра с подчёркивания:
...
Тут, обращаясь к полю, имя которого начинается с нижнего подчёркивания, мы сразу понимаем, что мы делаем что-то, не относящееся к публичному API класса. То есть залезаем во внутренности. Это уже нельзя сделать случайно и тут мы целенаправленно на свой страх и риск меняем что-то внутри класса, **понимая**, что можем всё сломать.
...
То есть это всё конкретно костыли.

Почему так? Потому что давайте-ка мы прочитаем ещё раз слово `dataclass`. Что это значит? Это значит класс данных. Это просто контейнер для данных. Это не класс для БЛ. Это просто контейнер для данных с прописанной структурой и всё. И для таких классов датаклассы отлично подходят. Что это за классы? Например, это Value Objects, то есть неизменяемые объекты, у которых нет идентичности:
...
Ещё датаклассы окей для DTO, Data Transfer Object. По сути, это просто контейнер данных для передачи этих данных между разными слоями приложения. Передаём данные не кортежами с кучей элементов, не обычными словарями, не типизированными словарями, а именно датаклассами. Причём надо стараться почаще использовать в таких случаях для датаклассов тоже параметры `frozen=True` и `slots=True`, потому что для DTO это естественно. Ну и конечно у DTO нет никакой бизнес-логики, это просто контейнер для данных с прописанной структурой и типизацией.
...
Во-вторых, основная задача Pydantic — это удобная работа с JSON, например, валидация входного JSON в вашем веб-сервисе. Для этого Pydantic отлично подходит и здесь он уместен в том же FastAPI или где угодно. Также и в обратную сторону, собрать JSON из вложенных сущностей удобно с Pydantic. Но использовать Pydantic где-то ещё я считаю неуместным.
