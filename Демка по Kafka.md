...
- не менее одного раза — мы не хотим терять сообщения, но можем получить дубликаты сообщения. С ними мы можем сами разобраться в наших потребителях, например, используя какие-то уникальные идентификаторы сообщений и запоминая, что мы уже обработали. То есть если обратка идемпотентна, то это наш выбо
- точно один раз — нам надо доставить строго 1 раз без потерь и дубликатов (есть мнение, что это на практике сложно выдержать в реальной системе, и что особо надеяться на это не стоит).