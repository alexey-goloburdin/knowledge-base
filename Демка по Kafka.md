...
Вот, собственно, и все основные концепции Apache Kafka.

Понятно, что есть готовые библиотеки и способы создать топик с нужным количеством реплик и разделов, записать туда данные, считать оттуда данные новые или старые, добавить брокеров в кластер, удалить брокер из кластера, добавить разделов топику и так далее.

Всё это легко гуглится, описано в книге и много где ещё. А концептуально понять эти базовые концепции, что это такое вообще и какие задачи решает, мне кажется важнее. Рад, что мы об этом поговорили — ну и переходим к следующей книге. Пока не знаю, какая она будет, но до связи в её комментариях. Пока-пока!