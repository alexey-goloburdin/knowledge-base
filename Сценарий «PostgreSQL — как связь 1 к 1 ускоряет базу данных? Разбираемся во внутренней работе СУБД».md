...
Тут `temp read` и `written` говорят о том, что использовались файлы для кеширования — потому что серверу не хватило рабочей памяти. Увеличим параметр `work_mem`, по умолчанию 4 мегабайта и это мало.
