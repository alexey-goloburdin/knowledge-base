...
Хочется, чтобы всё просто-удобно-красиво-приятно работало. И чтобы конечные клиентские программы, которыми пользуются люди, были простыми с привычным интерфейсом. Чтобы там не было сложных настроек, чтобы можно было просто скачать, установить, войти и пользоваться. Ну и конечно, нам нужен не аналог системы телеконференции, вроде Zoom, Яндекс.Телемоста и прочего. Нам нужен именно мессенджер с аудио и видеозвонками. Что-то похожее, например, на Telegram или WhatsApp, чтобы, например, созваниваться с родными. Поэтому разные системы, заточенные на конференции, вроде Jitsi, Nextcloud Talk и подобных, здесь не подходит.


И именно это мы сегодня с вами и сделаем.
