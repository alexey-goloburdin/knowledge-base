...
В комментариях написали, что это у тебя оно так работает, потому что ты проходишь по структуре и эффективнее проходить по данным, которые лежат на диске или в памяти последовательно, ибо кэши процессора и так далее. Конечно, последовательный доступ к рядом лежащим данным это хорошо, однако, однако в данном случае это не особо влияет на результаты, давайте в этом убедимся, вообще выбросив таблицу. Сейчас покажу код.
...
Именно поэтому я считал итерации цикла именно таким образом, как это было показано в прошлом видосе, потому что каждая итерация цикла сопряжена с действиями по обслуживанию цикла, то есть по работе с условием выхода из цикла, инкрементом счетчика и проверкой счетчика, не вышло ли его значение за установленным циклом пределы.

Некоторым комментаторам не понравилось, как я считал итерации и что вообще я назвал это итерациями. Но это итерации, на каждой из которых выполняются операции по обслуживанию цикла. Давайте сделаем свою версию **генератора** `range` и посмотрим, как она может работать:
...
Если подход внешнего меньшего цикла работает лучше при размерах 100 и 5, но хуже при размерах 50 и 1000, то отсюда можно сделать один простой вывод, что не надо нахрен об этом в питоне думать при написании вложенных циклов. Всё!

Если у вас есть конкретный вложенный цикл и по результатам профилирования вы выяснили, что он работает медленно и вам кровь из носу как надо его оптимизировать — ну вот как один из вариантов можете попробовать поменять их местами и сравнить результаты, стало лучше и это не погрешность — нормас, стало хуже — откатились обратно.
...
Но тем не менее, повторюсь, в случае питона подход с меньшим внешним циклом работает лучше не всегда, как мы убедились, а значит брать его в общем случае к использованию — не стоит.

Погонял аналогичные тесты на JS — результаты такие же, как и для питона, иногда выигрывает один сценарий, иногда другой.
